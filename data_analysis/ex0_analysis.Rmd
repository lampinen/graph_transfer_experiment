...
```{r}
crazy_rt_model = lmer(first_response_RT ~ condition * graph_trial_type * poly(time, 2) + transition_in_graph + target_combo + (1 + poly(time, 2) | subject_id) + (1 | target_str), 
                data=df_graph_trials %>%
                  filter(correct, 
                         !is.na(transition_in_graph),
                         graph_name == "fixed_random_graph_0",
                         first_response_RT < 5000))
summary(crazy_rt_model)
```
