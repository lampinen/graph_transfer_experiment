...
```{r}
in_graph_rt_model = lmer(first_response_RT ~ condition * graph_trial_type  + graph_name * log(time + 1) + target_combo + (1 + log(time + 1) | subject_id) + (1 | target_str), 
                data=df_graph_trials %>%
                  filter(correct, 
                         !is.na(transition_in_graph),
                         transition_in_graph,
...
```{r}
in_graph_rt_model_2 = lmer(first_response_RT ~ condition * graph_trial_type  + graph_name * poly(log(time + 1), 2) + target_combo + (1 + poly(log(time + 1), 2) | subject_id) + (1 | target_str), 
                data=df_graph_trials %>%
                  filter(correct, 
                         !is.na(transition_in_graph),
                         transition_in_graph,
...
```{r}
in_graph_correct_model = glmer(correct ~ condition * graph_trial_type  + graph_name * scale(log(time + 1)) + target_combo + (1 +scale(log(time + 1)) | subject_id) + (1 | target_str), 
                               family="binomial",
                               data=df_graph_trials %>%
                                 filter(!is.na(transition_in_graph),
                                        transition_in_graph))
